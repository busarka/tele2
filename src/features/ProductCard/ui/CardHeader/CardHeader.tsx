...
export const CardHeader: React.FC<CardHeaderProps> = ({ product, isSelected, isHovered, hasBeenHoveredAfterSelect }) => {
    return (
        <div className="product-card-header">
            ...
            <ul className="product-card-header__special">
                {product.special.map((item, index) => (
                    <li key={index} className="product-card-header__special-item">{boldNumbers(item)}</li>
                ))}
            </ul>
        </div>
    );
